...
function initGame() {
    ...
    if(flag==0) {
        quesbank = [];
        quesbank=Question.getAllByWeight(4, 3);
        ...
        for(var i=flag; i<5; i++) {
            if(i==flag) {
                quesbank.push(Question.getBySubCat(4, question.subslide));
            }
            else
                quesbank.unshift(Question.getByWeightExSubcat(4,question.subslide));
        }
    }
}
